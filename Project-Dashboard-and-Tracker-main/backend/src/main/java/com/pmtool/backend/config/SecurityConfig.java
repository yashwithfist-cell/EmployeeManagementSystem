    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests(authz -> authz
                        ...
                        .requestMatchers(HttpMethod.GET, "/api/projects/**", "/api/milestones/**", "/api/disciplines/**").authenticated()

                        // --- Employee-Specific Endpoints ---
                        .requestMatchers("/api/worklogs/my/**").hasRole("EMPLOYEE")
                        .requestMatchers("/api/leaves/**").hasAnyRole("EMPLOYEE","PROJECT_MANAGER","TEAM_LEAD","HUMAN_RESOURCE")
                        .requestMatchers("/api/attendancelog/**").hasAnyRole("EMPLOYEE", "SYSTEM_ADMIN","PROJECT_MANAGER","HUMAN_RESOURCE")
                        .requestMatchers("/api/salary-slips/**").hasRole("EMPLOYEE")
                        .requestMatchers("/api/systemattendance/**").hasAnyRole("EMPLOYEE", "SYSTEM_ADMIN","PROJECT_MANAGER","HUMAN_RESOURCE")
                        ...
                        .requestMatchers("/api/milestones/getUserMilestones").hasAnyRole("SYSTEM_ADMIN","PROJECT_MANAGER","TEAM_LEAD")
                        // Write operations for projects, milestones, and disciplines are admin-only.
                        .requestMatchers(HttpMethod.POST, "/api/projects/**", "/api/milestones/**", "/api/disciplines/**").hasRole("SYSTEM_ADMIN")
                        .requestMatchers(HttpMethod.PUT, "/api/projects/**", "/api/milestones/**", "/api/disciplines/**").hasRole("SYSTEM_ADMIN")
                        .requestMatchers(HttpMethod.DELETE, "/api/projects/**", "/api/milestones/**", "/api/disciplines/**").hasRole("SYSTEM_ADMIN")
